...
class Renderer {
    ...
    func getEntities() async -> [Entity] {
        ...
        if entities.isEmpty {
            entities = await createEntities()
        }
       
        return entities.map { $0.copy() as! Entity }
    }
}
...
extension Entity {
    func copy(with zone: NSZone? = nil) -> Any {
        let copy = Entity()
        copy.transform = self.transform // Make sure to copy the transform itself
        copy.components.set(ModelComponent(
            mesh: self.components[ModelComponent.self]!.mesh, // Deep copy the mesh
            materials: self.components[ModelComponent.self]!.materials // Reference the original materials for now
        ))
        return copy
    }
}
